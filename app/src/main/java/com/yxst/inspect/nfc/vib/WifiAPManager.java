package com.yxst.inspect.nfc.vib;
...
public class WifiAPManager {
	...
	//监听wifi热点的状态变化
    public static final String WIFI_AP_STATE_CHANGED_ACTION = "android.net.longsheng.WIFI_AP_STATE_CHANGED";
	...
	public boolean turnOnWifiAp(String str, String password, WifiSecurityType Type) {
		...
		if(Type == WifiSecurityType.WIFICIPHER_NOPASS) {
			if(DEBUG) Log.d(TAG, "longsheng ap----no password");
			...
			if(DEBUG) Log.d(TAG, "longsheng ap----wpa");
			//密码至少8位，否则使用默认密码
			...
			if(DEBUG) Log.d(TAG, "longsheng ap---- wpa2");
			//密码至少8位，否则使用默认密码
			if(null != password && password.length() >= 8){
				wcfg.preSharedKey = password;
			} else {
