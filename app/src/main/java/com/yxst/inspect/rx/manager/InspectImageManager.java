 ...
 */
public class InspectImageManager {

    public static boolean postInpsectImageToServer( final Context ctx,final List<InspectImage> imageList){
        Gson gson = new Gson();
        String jsonStr = gson.toJson(imageList);
        Observable<String> upImgRes = RestCreator.getRxRestService().postRecordImage(jsonStr);
                        ...
                        if(s!=null || !s.equals("")){
                            ...
                            if(resVal>0){
                               for(InspectImage image:imageList){
                                  InspectImage dbImage = InspectImageQueryUtil.getImageByURL(image.getLocalURL());
                                  dbImage.setIsUploadServer(ConfigInfo.UPLOAD_IMAGE);
                                  InspectImageQueryUtil.updateImage(dbImage);
                               }
                            }
                        }
...
//                Looper.loop();
    ...
    public static boolean upLoadImageByPath(String urlPath,String uploadPath){
        if("null".equals(uploadPath)||"".equals(uploadPath)){
            return false;
        }
        ...
        try {
            ...
            if(result.equals("true")){
                InspectImage image = InspectImageQueryUtil.getImageByURL(path);
                image.setIsUploadImage(ConfigInfo.UPLOAD_IMAGE);
                //   Log.e("image1",imagePaths.get(i).getPath()+","+result+","+image.getIsUploadImage());
                InspectImageQueryUtil.updateImage(image);
            }
//                        Looper.prepare();
